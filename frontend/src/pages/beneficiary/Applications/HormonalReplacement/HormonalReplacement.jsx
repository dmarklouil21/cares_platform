...
const HormonalReplacementStatus = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`/beneficiary/hormonal-replacement/list/`);
  ...
  const handleModalConfirm = async () => {
    if (modalAction?.type === "cancel") {
      try {
        ...
        setLoading(true);
        await api.delete(
          `/beneficiary/hormonal-replacement/cancel-request/${modalAction.id}/`
        );
  ...
  const handleView = (id) => {
    navigate(`/beneficiary/applications/hormonal-replacement/view/${id}`);
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 w-full">
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-yellow-600">
              Track the status of your medication requests here.
            </h3>
          </div>
...
export default HormonalReplacementStatus;
