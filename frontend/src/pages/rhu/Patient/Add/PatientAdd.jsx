...
// --- Reusable UI Helpers (Defined OUTSIDE the component to fix focus issues) ---
...
const PatientMasterListAdd = () => {
  ...
  // --- Fetch Profile (to set registered_by) ---
  const fetchProfile = async () => {
    try {
        ...
        setForm((prev) => ({
            ...prev,
            registered_by: data?.lgu || data?.rhu_name // Fallback depending on API response structure
        }));
    } catch (e) {
        console.error("Failed to load RHU profile", e);
    }
  };
  ...
  const validate = () => {
    ...
    // Validate BOTH emergency contacts
        ...
        <div className="flex flex-col mb-6">
            ...
            {loggedRepresentative && (
                <p className="text-sm text-gray-500">
                    Registering under RHU: <span className="font-semibold text-primary">{loggedRepresentative.rhu_name || loggedRepresentative.lgu}</span>
                </p>
            )}
        </div>
