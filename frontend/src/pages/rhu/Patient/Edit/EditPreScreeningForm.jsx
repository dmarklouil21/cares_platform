...
const PatientPreScreeningForm = () => {
  ...
  const photoUrl = location.state?.photoUrl; // if needed for re-upload logic
  ...
  useEffect(() => {
    if (record) {
      // Determine data source (Add Flow vs Edit Flow)
      const patientData = record.formData || record.patient; 
      const preScreeningData = record.formData?.pre_screening_form || record.patient?.pre_screening_form || {};
      
      setPatient(patientData);
      setForm(preScreeningData);
    }
  }, [record]);
  ...
  const handleCheckboxChange = (groupName, value) => {
    setForm(prev => {
      ...
      let updated;
      if (exists) {
        updated = current.filter(item => item.name !== value);
      } else {
        updated = [...current, { name: value }];
      }
  ...
  const handleSubmit = (e) => {
    ...
    setModalDesc("Confirm update of pre-screening data.");
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
        ...
        // Prepare payload (clean up dates if empty strings)
        ...
        // If we were passed a photo from the previous step (Add flow), include it
        if (photoUrl) {
        ...
        setTimeout(() => navigate("/rhu/patients"), 1500);
      ...
      <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-6 w-full bg-white rounded-lg py-7 px-5 md:px-8 flex-1 overflow-auto shadow-sm">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-sm text-gray-500">
                            Pre-Screening Information for <span className="font-semibold text-gray-700">{patient.full_name}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                <button
                    type="button"
                    onClick={() => navigate(`/rhu/patients/edit/${patient.patient_id}`)}
