...
const PatientPreScreeningForm = () => {
  ...
  useEffect(() => {
    if (record) {
      // Handle data structure (formData if coming from Add flow, patient if from View flow)
      const pData = record.formData || record.patient;
      if (pData) {
        setPatient(pData);
        setForm(pData.pre_screening_form || {});
      }
    }
  }, [record]);
      ...
      <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-6 w-full bg-white rounded-lg py-7 px-5 md:px-8 flex-1 overflow-auto shadow-sm">
          ...
          <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
            <button
              onClick={() => navigate(`/rhu/patients/view/${patient.patient_id}`)}
