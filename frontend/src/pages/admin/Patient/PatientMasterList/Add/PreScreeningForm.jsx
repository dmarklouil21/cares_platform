  ...
  <div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
      {options.map((opt) => {
         ...
         const isChecked = Array.isArray(currentValues) && currentValues.some(item => 
            (typeof item === 'object' ? item.name : item) === opt
         );
...
const PatientPreScreeningForm = () => {
  ...
  const photoUrl = location.state?.photoUrl; // File object
  ...
  const [form, setForm] = useState({
    ...
    chief_complaint: "",
    date_of_diagnosis: "",
    diagnosis_basis: [], // Array of objects {name: "..."} or strings
    primary_sites: [],
    primary_sites_other: "",
    laterality: "",
    multiple_primaries: "",
  ...
  useEffect(() => {
    if (!generalData) {
      // If user refreshes or accesses directly, redirect back
      navigate("/admin/patient/add/general-data");
    }
  ...
  const handleCheckboxChange = (groupName, value) => {
    setForm(prev => {
      ...
      // Note: Keeping structure simple (array of objects with 'name' key based on backend requirement in original code)
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
        ...
        // Prepare Payload
        // Ensure checkbox arrays are cleaned if needed, but structure [{name: "Val"}] works based on previous code
  ...
  return (
    <div className="w-full h-screen bg-gray flex flex-col overflow-auto">
      ...
      <ConfirmationModal
        ...
        desc="Are you sure all information is correct? This will create a new patient record."
      ...
      <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-6 w-full bg-white rounded-lg py-7 px-5 md:px-8 flex-1 overflow-auto shadow-sm">
            ...
            <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                <button
                    type="button"
                    onClick={() => navigate("/admin/patient/add/general-data", { state: { formData: generalData, photoUrl: photoUrl } })}
