// src/pages/cancer-management/generate/generate.jsx
import React from "react";

export default function CancerManagementPrint({ rows = [] }) {
  // Safer date parsing (treat YYYY-MM-DD as local midnight to avoid TZ drift)
  const fmtDate = (v) => {
    if (!v) return "";
    try {
      const d =
  ...
  return (
    <div
      id="cancermanagement-print-content"
    ...
    >
      <style>{`
        ...
        @media print {
          ...
          #cancermanagement-print-content {
      ...
      <div className="flex-1 flex flex-col px-24 pt-32">
        <div className="py-5 w-full text-center">
          <h1 className="font-bold text-xl">Cancer Management Requests</h1>
        ...
        <table>
          <colgroup>
            <col style={{ width: "18%" }} /> {/* Patient ID */}
            <col style={{ width: "26%" }} /> {/* Name */}
            <col style={{ width: "20%" }} /> {/* Service Requested */}
            <col style={{ width: "16%" }} /> {/* Date Submitted */}
          ...
          <thead>
            <tr>
              ...
              <th className="text-left">Service Requested</th>
          ...
          <tbody>
            {rows.length ? (
              rows.map((item) => (
                <tr key={item.id ?? item.patient?.patient_id ?? Math.random()}>
                  ...
                  <td className="text-left">{item.service_type ?? "—"}</td>
                  <td className="text-center">
