...
const PatientMasterListEdit = () => {
  ...
  useEffect(() => {
    return () => {
      if (photoUrl && imageFile) URL.revokeObjectURL(photoUrl);
    };
  }, [photoUrl, imageFile]);
  ...
  const validate = () => {
    ...
    // Validate emergency contacts (Both Required as per request)
    form.emergency_contacts.forEach((contact, index) => {
        if (!contact.name.trim()) 
            newErrors[`emergency_contact_${index}_name`] = "Required";
        
        if (!contact.relationship_to_patient.trim()) 
            newErrors[`emergency_contact_${index}_relationship`] = "Required";
        
        if (!contact.address.trim()) 
            newErrors[`emergency_contact_${index}_address`] = "Required";
        
        if (!contact.email.trim()) 
            newErrors[`emergency_contact_${index}_email`] = "Required";
        
        if (!contact.mobile_number.trim()) 
            newErrors[`emergency_contact_${index}_mobile_number`] = "Required";
    });
  ...
  const handleNext = () => {
    ...
    navigate(`/private/patients/edit/${patient_id}/cancer-data`, {
      state: {
        formData: { ...form },
        photoUrl: imageFile, 
      },
      ...
      <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-8 w-full bg-white rounded-lg py-8 px-6 md:px-10 shadow-sm border border-gray-100 flex-1">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start border-b border-gray-100 pb-8">
                ...
                <div className="flex-1 text-center md:text-left">
                    <h1 className="text-2xl font-bold text-gray-800">
                        {form.first_name || "Patient"} {form.last_name || "Name"}
                    </h1>
            ...
            <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                <button
                    type="button"
                    onClick={() => navigate("/private/patients")}
