...
const CheckboxGroup = ({ label, groupName, options, currentValues, onChange, error }) => (
  <div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
      {options.map((opt) => {
         // Check if value exists in array (handling both string array or object array structure)
         const safeValues = Array.isArray(currentValues) ? currentValues : [];
         const isChecked = safeValues.some((item) => (item.name || item) === opt);
...
const ViewPreScreeningForm = () => {
  ...
  const record = location.state; // Contains { formData, photoUrl } if coming from Add, or { patient } if from Edit
  ...
  useEffect(() => {
    if (record) {
      // Determine source: Add Flow (formData) vs Edit Flow (patient)
      const patientData = record.formData || record.patient;
      const formData = record.formData?.pre_screening_form || record.patient?.pre_screening_form || {};
      const pUrl = record.photoUrl; // For Add flow
  ...
  const handleCheckboxChange = (groupName, value) => {
    setForm((prev) => {
      ...
      const exists = currentValues.some((item) => (item.name || item) === value);
      
      let updatedValues;
      if (exists) {
        // Remove
        updatedValues = currentValues.filter((item) => (item.name || item) !== value);
      } else {
        // Add (maintaining object structure {name: val})
        updatedValues = [...currentValues, { name: value }];
      }
  ...
  const handleSave = (e) => {
    ...
    setModalDesc("Are you sure you want to update the pre-screening form?");
  ...
  const handleModalConfirm = async () => {
    if (modalAction?.type === "submit") {
      ...
      try {
        ...
        // Note: Using form directly, which contains the checkbox arrays in {name: val} structure
        formData.append("cancer_data", JSON.stringify(form));
        formData.append("general_data", JSON.stringify(patient));
        
        // Only append photo if it's being updated/added
        if (photoUrl) {
            formData.append("photo_url", photoUrl);
        }

        // Correct Endpoint: Patient Update
        await api.patch(
        ...
        navigate("/private/patients", { state: { flash: "Patient data updated successfully!" }});
  ...
  if (!form || !patient) return <SystemLoader />;
        ...
        <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-7xl mx-auto w-full">
            ...
            <div className="flex flex-col gap-6 w-full bg-white rounded-lg py-7 px-5 md:px-8 flex-1 overflow-auto shadow-sm">
                ...
                <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                    <button
                        type="button"
                        onClick={() => navigate(`/private/patients/edit/${patient.patient_id}`)}
