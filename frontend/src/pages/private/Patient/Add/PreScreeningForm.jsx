...
const CheckboxGroup = ({ label, groupName, options, currentValues, onChange, error }) => (
  <div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
      {options.map((opt) => {
         // Handle structure: array of objects {name: "Value"}
...
const PatientPreScreeningForm = () => {
  ...
  const [form, setForm] = useState({
    ...
    chief_complaint: "",
    date_of_diagnosis: "",
    diagnosis_basis: [], // Array of objects {name: "..."}
    primary_sites: [],
    primary_sites_other: "",
    laterality: "",
    multiple_primaries: "",
  ...
  useEffect(() => {
    if (!generalData) {
        // Redirect if accessed directly without step 1 data
        navigate("/private/patients/add");
    }
  ...
  const handleCheckboxChange = (groupName, value) => {
    setForm((prev) => {
      ...
      const updatedValues = exists
        ...
        : [...currentValues, { name: value }]; // Add as object {name: val}
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
        ...
        if (photoUrl) {
            formData.append("photo_url", photoUrl); // The actual File object from Step 1
        }
        ...
        // Navigate on success
        // Assuming "/private/patients" is the master list
        navigate("/private/patients", { state: { flash: "Patient added successfully!" } });
        ...
        <div className="py-5 px-5 md:px-5 flex flex-col flex-1 max-w-7xl mx-auto w-full">
            ...
            <div className="flex flex-col gap-6 w-full bg-white rounded-lg py-7 px-5 md:px-8 flex-1 overflow-auto shadow-sm">
                ...
                <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                    <button
                        type="button"
                        onClick={() => navigate("/private/patients/add")}
