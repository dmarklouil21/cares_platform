...
const PatientMasterListAdd = () => {
  ...
  const [form, setForm] = useState({
    ...
    source_of_income: "",
    monthly_income: "",
    registered_by: "", // Will be populated by fetchProfile
    emergency_contacts: [
      { ...DEFAULT_CONTACT },
      { ...DEFAULT_CONTACT },
    ],
  ...
  const fetchProfile = async () => {
    try {
        ...
        // Update form immediately with the fetched institution name
        setForm((prev) => ({
            ...prev,
            registered_by: data?.institution_name || ""
        }));
    } catch (error) {
  ...
  const handleNext = () => {
    ...
    navigate(`/private/patients/add/cancer-data-form`, {
      state: {
        formData: form,
        photoUrl: imageFile,
      },
        ...
        <div className="flex flex-col mb-6">
            ...
            {loggedRepresentative && (
                <p className="text-sm text-gray-500">
                    Registering under: <span className="font-semibold text-primary">{loggedRepresentative.institution_name}</span>
                </p>
            )}
        </div>
