...
const PreCancerousView = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const { data } = await api.get(`/partners/pre-cancerous/view/${id}/`)
        if (!mounted) return;
  ...
  const handleModalConfirm = async () => {
    try {
      ...
      // Stop here for now
      await api.patch(`/partners/pre-cancerous/update/${preCancerous.id}/`, payload);
      ...
      <div className="h-screen w-full flex flex-col p-5 gap-3 justify-start items-center bg-gray overflow-auto">
        ...
        <div className="w-full flex flex-col md:flex-row gap-3 justify-between md:justify-around">
          <Link
            ...
            to="/private/treatment-assistance/pre-cancerous"
          >
            BACK
          </Link>
